...
type BeliefStatementSlideProps = Pick<SlideRenderContext, 'isThumbnail' | 'step'>

export function BeliefStatementSlide({
  isThumbnail = false,
  step = 0,
}: BeliefStatementSlideProps) {
  const copy = STEP_COPY[Math.min(step, STEP_COPY.length - 1)]

  return (
    <article className="belief-statement-slide">
